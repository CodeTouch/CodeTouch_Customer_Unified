...
function settingElements(tag) {
    ...
    const textContentInput = document.getElementById('letterText');
...
// ///////////////////오른쪽 설정///////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        // 글자 변경
        if (elemStyle.content) {
            const E_content = (e) => {
                // getComputedStyle은 읽기 전용이므로, 실제 스타일 변경은 clickElem에 적용
                targetNode.innerText = `${e.target.value}`; // content 값은 반드시 ""로 감싸야 함
            };
            console.log("이거")
            if (!handlers[targetNode + '-content']) {
                textContentInput.addEventListener('input', E_content);
                handlers[targetNode + '-content'] = E_content;
            } else {
                textContentInput.removeEventListener('input', handlers[targetNode + '-content']);
                textContentInput.addEventListener('input', E_content);
                handlers[targetNode + '-content'] = E_content;
            }
        }
