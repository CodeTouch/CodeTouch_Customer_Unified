...
public interface ProductMapper {
    //생성
    @Insert("INSERT INTO product (site_id, name, category, price, sale_percentage, sale_name, sale_period, image, stock, description) VALUES (#{site_id}, #{name}, #{category}, #{price}, #{sale_percentage}, #{sale_name}, #{sale_period}, #{image}, #{stock}, #{description})")
    int insertProduct(ProductDTO dto);
    //수정 -> 의문) site와 product는 연결되어 있다만 조건으로 siteId를 같이 부여? or not?
    @Update("UPDATE product SET name=#{name}, category=#{category}, price=#{price}, sale_percentage=#{sale_percentage}, sale_name=#{sale_name}, sale_period=#{sale_period}, image=#{image}, stock=#{stock}, description=#{description} WHERE site_id=#{site_id} AND pd_id=#{pd_id}")
    int updateProduct(UpdateProductDTO dto);
    //삭제
    @Delete("DELETE FROM product WHERE site_id=#{site_id} AND pd_id=#{pd_id}")
    int deleteProduct(UseProductDTO dto);
    //특정 상품에 대한 모든 정보 출력
    @Select("SELECT name, category, price, sale_percentage, sale_name, sale_period, image, description FROM product WHERE site_id=#{site_id} AND pd_id=#{pd_id}" )
    List<ProductDTO> findAllById(UseProductDTO dto);
    //특정 상품에 대한 이름과 가격 출력
